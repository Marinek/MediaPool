...
public class BOMovie extends BOAbstractMedia {
	...
	private MovieVO getMovieVO() throws MPExeption {
		...
		movieVO.setId(this.getCurrentMediaBean().getId());
		movieVO.setLengthMinutes(this.getCurrentMediaBean().getLength());
		movieVO.setName(this.getCurrentMediaBean().getName());
	...
	private AbstractMediaBean getMediaBean(MovieVO movieVO) {
		...
		movieBean.setPersistentStatus(PersistentStatus.PERSISTENT);
