...
public class MediaService implements Serializable {
	...
	// Persistence.createEntityManagerFactory(PERSISTENCE_UNIT).createEntityManager();
	// em.getTransaction().begin();
	// MovieContainer migratedFilms = new
	// MovieContainer(MovieHoldingEntry.class,
	// MovieEntryType.MOVIEHOLDINGENTRY);
	...
	// holding.setKnown("Ja".equalsIgnoreCase(film.getGesehen()) ? "known" :
	// "unknown");
	...
	// MovieContainer movieEntrys = new MovieContainer(MovieEntry.class,
	// MovieEntryType.MOVIEENTRY);
	...
	// MovieContainer movieEntryItems = new
	// MovieContainer(MovieHoldingEntry.class,
	// MovieEntryType.MOVIEHOLDINGENTRY);
	//
	// EntityManager em =
	...
	// em.getTransaction().begin();
	// Query q =
	// em.createQuery("SELECT m FROM MUser m where m.email=:email and m.password=:password");
	...
	// MovieContainer movieProductEntries = new
	// MovieContainer(MovieProductEntry.class,
	...
	// em.getTransaction().begin();
	// Query q =
	// em.createQuery("SELECT p FROM Product p where p.movie.title=:title");
	...
	// em.getTransaction().begin();
	// Query q =
	// em.createQuery("SELECT p FROM Mpresets p where p.field=:field");
	...
	// MovieContainer movieItems = new MovieContainer(MovieEntry.class,
	// MovieEntryType.MOVIEENTRY);
	// EntityManager em =
	// Persistence.createEntityManagerFactory(PERSISTENCE_UNIT).createEntityManager();
	// em.getTransaction().begin();
	...
	// BeanItem<MovieHoldingEntry> newMovieEntryItem =
	// (BeanItem<MovieHoldingEntry>) item;
	// Holding holding = newMovieEntryItem.getBean().getHolding();
	// String newUrl = saveImage(holding.getProduct().getImage(),
	// holding.getProduct().getEan() + "");
	...
	// }
	//
	// public void removeRating(MRating mrating) {
	// EntityManager em =
	// Persistence.createEntityManagerFactory(PERSISTENCE_UNIT).createEntityManager();
	// em.getTransaction().begin();
	// MRating toremove = em.merge(mrating);
	// em.remove(toremove);
	// em.getTransaction().commit();
	// }
	//
	...
	// }
	//
	// final static String actors[] = { "Marlon Brando", "Al Pacino",
	// "Robert DeNiro", "Leonardo DiCaprio" };
	// final static String movies[] = { "Der Pate I", "Der Pate II",
	// "Der Pate III", "Inception" };
	// final static String covers[] = { "Pate.jpg", "Pate_II.jpg",
	// "Pate_III.jpg", "Inception.jpg" };
	//
