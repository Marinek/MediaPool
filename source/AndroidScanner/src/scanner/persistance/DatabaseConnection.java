...
/** 
 * Class to insert data into database. Data means in this content clients authorization keys.
 * the given data is bound to an account. Furthermore the consumer information can be deposited and called.
 * 
 * @author Stefan Braun
 *
 */
public class DatabaseConnection  implements DatabaseConfig {
	  ...
	  private DatabaseOpenHelper dbh;
		 ...
		 * @param accessTokenPair accesstokenpair to connect the user of the application with the webservice
		 * @throws CouldNotWriteIntoDatabaseException if the database cannot be opened for writing
		 */
		 ...
		 * @return keySecretPair a list containing the consumerkey and the consumersecret
		 * @throws CouldNotReadDatabaseException if the database cannot be opened
		 */
		public String[] getKeySecretPair() throws CouldNotReadDatabaseException{
			...
			try{
				...
				Cursor mCursor = this.db.query(true, TABLE_NAME_CONSUMER, new String[] {
		 ...
		 * @param accountName name of users account
		 ...
		 * @param keySecretPair a list containing the consumerkey and secret
		 * @throws CouldNotWriteIntoDatabaseException if the database cannot be opened for writing
		 */
		 ...
		 * @param accountName name of users account
		 * @param accessTokenPair  a list containing accesstoken and accesssecret
		 * @throws CouldNotWriteIntoDatabaseException if the database cannot be opened for writing
		 */
		...
		public ArrayList<String> getAccounts()throws CouldNotReadDatabaseException{
			...
			try{
				this.db=this.dbh.getReadableDatabase();
				Cursor mCursor = this.db.query(true, TABLE_NAME_USER, new String[] {
				USER_NAME},
				null, null, null, null, null, null);
		 ...
		 * 
		 * @param accountName Screen name of the account to check.
		 * @return <code>true</code> if the account is contained in the database;
		 *           <code>false</code> otherwise.
		 * @throws CouldNotReadDatabaseException If the application's database could not be opened.
		 */
		public boolean isAccountExisting(String accountName) throws CouldNotReadDatabaseException {
		  ...
		  try {
		    this.db = this.dbh.getReadableDatabase();
		    Cursor cursor = this.db.query(TABLE_NAME_USER, null,
                                      USER_NAME+"='"+accountName+"'", null, null,
                                      null, null);
     ...
     */
		public boolean isKeySecretPairExisting() throws CouldNotReadDatabaseException {
		  ...
		  try {
		    this.db = this.dbh.getReadableDatabase();
		    Cursor cursor = this.db.query(TABLE_NAME_CONSUMER, null, null, null, null,
		                                  null, null);
		    if(cursor.getCount() > 0)
		      { result = true; }
		    cursor.close();
		 ...
		 * @param eanNumber Ean code to be saved
		...
		/**
		 * gets all eanNumber with isSynchronized T out of database and returns them in an ArrayList
		 * @param isState the state of isSynchronized T or F
		 ...
		 * @param numbers a list containing the described numbers
		 * @param flag the flag it should be set to 
		 * @throws CouldNotReadDatabaseException if the database cannot be opened
		 */ 
		 ...
		 * @param code the code to look after. 
